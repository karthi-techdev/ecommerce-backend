...
router.post("/",(req, res, next) => {(req as any).managementName = "products";next();},upload.fields([{ name: "images", maxCount: 10 },{ name: "thumbnail", maxCount: 1 }]),productController.createProduct);
router.get('/',(req, res, next) => productController.getProducts(req, res, next));
router.post('/checkSlugExist', (req, res, next) => productController.checkSlugExist(req, res, next));
router.get('/getProductById/:id',(req, res, next) => productController.getProductById(req, res, next));
router.put('/updateProduct/:id', (req, res, next) => {(req as any).managementName = "products";next();},upload.fields([{ name: "images", maxCount: 10 },{ name: "thumbnail", maxCount: 1 }]), productController.updateProduct);
