...
const SliderListTemplate: React.FC = () => {
  const navigate = useNavigate();
  const {
    sliders,
    fetchAllSlider,
    deleteSlider,
    toggleSliderStatus,sliderstats,stats,
    totalPages,
    loading,
    error,
  } = useSliderStore();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        await fetchAllSlider(currentPage, PAGINATION_CONFIG.DEFAULT_LIMIT, selectedFilter);
        await sliderstats();
        console.log(totalPages,'total pages here')
      } catch (err: any) {
        toast.error(err?.message || 'Failed to load Sliders. Please try again.');
      }
    };
    loadData();
  }, [currentPage, selectedFilter, fetchAllSlider]);
  ...
  const filteredSliders = sliders.filter((slider) => {
    // Apply status filter
    if (selectedFilter === 'active' && slider.status !== 'active') return false;
    if (selectedFilter === 'inactive' && slider.status !== 'inactive') return false;

    // Apply search filter
    return slider.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      slider.highlightsText.toLowerCase().includes(searchTerm.toLowerCase());
  });
  ...
  const handleToggleStatus = async (slider: Slider) => {
    ...
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: `Do you want to ${action} this Slider?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await toggleSliderStatus(slider._id!);
        // Refresh the data to ensure UI is in sync with backend
        await fetchAllSlider(currentPage, PAGINATION_CONFIG.DEFAULT_LIMIT, selectedFilter);
        await sliderstats();
        toast.success(`Slider ${action}d successfully!`);
      } catch (error) {
  ...
  const handleDelete = (slider: Slider) => {
    Swal.fire({
      title: 'Are you sure?',
      text: `You are about to delete "${slider.title}"`,
      icon: 'warning',
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        try {
          ...
          const updatedLength = filteredSliders.length - 1;
          const newTotalItems = sliders.length - 1;
          ...
          if (updatedLength === 0 && currentPage > newTotalPages) {
            ...
            await fetchAllSlider(newPage, PAGINATION_CONFIG.DEFAULT_LIMIT, selectedFilter);
            await sliderstats();
          } else {
          ...
          Swal.fire('Deleted!', 'The Slider has been removed.', 'success');
        } catch (error) {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredSliders.length === 0 ? (
                ...
                filteredSliders.map((slider: Slider, index: number) => (
                  <tr key={slider._id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {(currentPage - 1) * PAGINATION_CONFIG.DEFAULT_LIMIT + index + 1}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {truncate(slider.title, 40)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      <img src={`http://localhost:5000${slider.image}`}
  alt={slider.title}
  className="h-10 w-10 object-cover rounded"/>
                    ...
                    <td className="px-4 py-2">
                      <button
                        ...
                        className={`${slider.status === 'active' ? 'text-green-500 hover:text-green-700' : 'text-gray-400 hover:text-gray-600'} transition`}
                        title="Toggle Status"
                      >
                        {slider.status === 'active' ? <ToggleRight size={18} /> : <ToggleLeft size={18} />}
                      </button>
                    </td>
                    <td className="px-4 py-2 flex gap-3 items-center">
                      <button
                        onClick={() => navigate(`/slider/edit/${slider._id}`)}
...
export default SliderListTemplate;