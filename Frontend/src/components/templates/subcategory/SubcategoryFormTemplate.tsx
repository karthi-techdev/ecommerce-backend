...
//         description: subCategory.description || '',
//         mainCategoryId: subCategory.mainCategoryId || '',
//         image: null, // Keep as null during edit unless a new file is picked
//       });
//       setSelectedMainCategoryName({
//         label: subCategory.mainCategory?.name,
//         value: subCategory.mainCategoryId,
...
//       // Explicitly append fields to ensure control over the payload
//       payload.append('name', formData.name);
//       payload.append('slug', formData.slug);
//       payload.append('description', formData.description || '');
//       payload.append('mainCategoryId', formData.mainCategoryId);

//       // Only append image if it's a new file selection
...
//           <div>
//             <div className="h-24 w-24 border border-gray-300 rounded flex items-center justify-center overflow-hidden bg-gray-50">
//               <img src={imagePreview || previewImage} className="h-full w-full object-cover" alt="Preview" />
//             </div>
//           </div>
//         </div>

//         <div className="mt-6 flex justify-end">
//           <button type="submit" disabled={isSubmitting} className="px-4 py-2 bg-amber-500 text-white rounded hover:bg-amber-600 disabled:bg-gray-400 transition-colors">
//             {isSubmitting ? 'Saving...' : id ? 'Update' : 'Save'}
//           </button>
//         </div>
//       </form>
...
const SubCategoryFormTemplate: React.FC = () => {
  ...
  const handleChange = (e: any) => {
    ...
    // Clear error for the specific field that was touched
    setErrors(prev => ({ ...prev, [name]: undefined }));

    const validation = validateSubCategoryForm(updatedFormData, !!id);
    setErrors(prev => ({...prev, [name]: validation[name as keyof ValidationErrors],}));

    // FIX: Re-run duplicate check if either NAME or MAIN CATEGORY changes
    if ((name === 'name' || name === 'mainCategoryId') && updatedFormData.name.trim() && updatedFormData.mainCategoryId) {
      debounceNameCheck(async () => {
        try {
          ...
          if (exists) {
            ...
            // Explicitly clear name error if it's unique in the selected category
            setErrors(prev => ({ ...prev, name: undefined }));
          }
        } catch (error: any) {
          setErrors(prev => ({...prev, name: handleError(error)?.[0], }));
        ...
        <div className="space-y-6">
          {subCategoryFields.map(field => {
            ...
            return (
              ...
              field={{
                ...field,
                options: field.name === 'mainCategoryId'
                  ? (() => {
                      const opts = subCategories?.filter((cat) => cat && cat._id && cat.name).map((cat) => ({
                        label: cat.name,
                        value: cat._id || "",
                      })) || [];
                      if (formData.mainCategoryId && !opts.find((o) => o.value === formData.mainCategoryId)) {
                        opts.push(selectedMainCategoryName);
                      }
                      return opts;
                    })()
              ...
              isRequired={isRequired}
              value={field.type === 'file' ? undefined : formData[field.name as keyof SubCategoryFormData] ?? ''}
              onChange={handleChange}
              error={errors[field.name as keyof ValidationErrors]}
            />
            );
