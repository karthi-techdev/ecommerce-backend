...
import { HiOutlineUser } from 'react-icons/hi';
import { motion, AnimatePresence } from 'framer-motion';
import logoImg from '../../assets/images/logo.png'; // Fallback static logo
import LabeledInput from '../molecules/LabeledInput';
import ProfileDropdown from '../organisms/ProfileDropdown';
import NotificationList from '../molecules/NotificationList';
import { useAuthStore } from '../../stores/authStore';
import { useSettingsStore } from '../../stores/settingsStore'; // Import Store
import ImportedURL from '../../common/urls'; // Import URL config
import Swal from 'sweetalert2';
...
const Navbar = () => {
  ...
  // 3. Compute Dynamic ADMIN Logo URL
  const dynamicAdminLogo = useMemo(() => {
    // We target the adminLogo specifically here
    const logoPath = settings?.branding?.adminLogo; 
    ...
    if (logoPath) {
      ...
      // even if the filename remains the same on the server
      return `${baseUrl}${cleanPath}?t=${new Date().getTime()}`;
    }
    
    return logoImg; // Fallback to local asset
  }, [settings?.branding?.adminLogo]); // Dependency on adminLogo
    ...
    >
      <div className="px-4 sm:px-4 lg:px-4 shadow-sm pl-15 md:pl-4">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex-shrink-0 flex items-center group">
              {/* 4. Display the Admin Logo with a key to force re-render */}
              <img 
                key={dynamicAdminLogo} 
                src={dynamicAdminLogo} 
                alt="admin logo" 
          ...
          <div className="flex items-center">
            ...
            <div className="ml-2 relative">
              ...
              <AnimatePresence>
                {notificationDropdownOpen && (
                  <>
                    <button className="fixed inset-0 z-40" onClick={closeAllDropdowns}></button>
                    <motion.div
                      initial={{ opacity: 0, y: 20 }}
                      animate={{ opacity: 1, y: 0 }}
                      exit={{ opacity: 0, y: 20 }}
            ...
            <div className="ml-4 relative">
              ...
              >
                <motion.div
                  whileHover={{ scale: 1.05 }}
                  className="h-8 w-8 rounded-full bg-amber-500 flex items-center justify-center shadow-sm"
                >
                  <HiOutlineUser className="h-4 w-4 text-white" />
                </motion.div>
                <span className="hidden md:inline-block ml-2 text-sm font-medium text-gray-700">
              ...
              <ProfileDropdown isOpen={profileDropdownOpen} onClose={closeAllDropdowns} onSignOut={handleSignOut} />
            </div>
          </div>
        </div>
      </div>
